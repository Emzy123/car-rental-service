...
import { CheckCircle2, Fuel, Settings2, Users, Luggage } from 'lucide-react';
...
export default function VehicleDetailPage() {
  ...
  if (error || !data?.vehicle) return <p className="py-20 text-center text-error">{error?.message || 'Vehicle not found'}</p>;

  const { vehicle, available_for_dates } = data;
        ...
        <div className="lg:col-span-2">
          ...
          <div className="mt-6 flex flex-wrap items-start gap-3">
            <div className="flex-1">
              ...
              <p className="mt-1 text-sm text-gray-500">{vehicle.year} · {vehicle.license_plate}</p>
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
            {specs.map((s) => (
              <div key={s.label} className="flex flex-col items-center rounded-xl border border-gray-100 bg-gray-50 p-4">
                ...
                <p className="mt-0.5 text-sm font-semibold capitalize text-primary-500">{s.value}</p>
              </div>
            ))}
          </div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {vehicle.features.map((f) => (
                  <span key={f} className="inline-flex items-center gap-1.5 rounded-full border border-gray-200 bg-white px-3 py-1.5 text-sm text-gray-700">
                    <CheckCircle2 className="h-3.5 w-3.5 text-secondary-500" />{f}
                  </span>
                ))}
              </div>
            </div>
          ...
          <div className="mt-8 rounded-xl bg-primary-50 p-5">
            ...
            <ul className="mt-3 space-y-2 text-sm text-gray-700">
              {['Basic insurance coverage', '24/7 roadside support', 'Free cancellation (48h+ before pickup)', 'Unlimited mileage on selected vehicles'].map((item) => (
